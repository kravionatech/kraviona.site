...
export default function TiptapEditor({
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: { class: "canvas", "aria-label": "Article content" },
    },
  ...
  const upload = (file?: File) => {
    ...
    if (!file.type.startsWith("image/") || file.size > 7 * 1024 * 1024)
      return setError("Choose a JPG, PNG, WebP or AVIF image under 7 MB.");
    setUploading(true);
    setError("");
  ...
  const tool = (label: string, action: () => void, active = false) => (
  ...
  return (
    <div className="rich-editor tiptap-editor">
      <div className="toolbar" role="toolbar" aria-label="Rich text formatting">
        {!preview && (
          <>
            ...
            {tool("Link", link, editor.isActive("link"))}
            {tool(uploading ? "Uploading…" : "Image", () =>
            ...
            {tool("Undo", () => editor.chain().focus().undo().run())}
            {tool("Redo", () => editor.chain().focus().redo().run())}
          </>
        )}
        <button
          type="button"
      ...
      <div
        className={`editor-footer ${words >= maxWords ? "danger-text" : ""}`}
      >
        <span>
          {words} / {maxWords} words
        </span>
        <span>{preview ? "Read-only preview" : "Autosynced"}</span>
      </div>
      {error && (
