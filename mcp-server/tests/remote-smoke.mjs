...
if (!origin || !accessKey) throw new Error('MCP_PUBLIC_URL and MCP_BEARER_TOKEN are required');

const endpoint = `${origin}/mcp`;
console.log('1/7 OAuth discovery');
const metadata = await fetch(`${origin}/.well-known/oauth-authorization-server`).then(async response => {
...
if (!registrationResponse.ok || !registration.client_id) {
  throw new Error(`DCR failed (${registrationResponse.status}): ${JSON.stringify(registration)}`);
}
...
if (consent.status !== 200 || !requestId) throw new Error('Consent screen did not return an authorization request ID');
...
if (approval.status !== 302 || !code) throw new Error('Approval did not issue an authorization code');
...
if (!oauthResponse.ok || !oauth.access_token) {
  throw new Error(`Token exchange failed (${oauthResponse.status}): ${JSON.stringify(oauth)}`);
}
...
const transport = new StreamableHTTPClientTransport(new URL(endpoint), {
  ...
  fetch: async (url, init) => {
    ...
    console.log(`MCP ${init?.method || 'GET'} -> ${response.status} ${response.headers.get('content-type') || ''}`);
    return response;
  }
});
const client = new Client({ name: 'kraviona-production-smoke', version: '1.0.0' });
...
const posts = await client.callTool({ name: 'list_posts', arguments: { status: 'all', limit: 1 } });
await client.close();
...
console.log(`Production OAuth MCP passed: DCR, PKCE, token exchange, ${tools.tools.length} tools, health and authenticated posts read.`);
