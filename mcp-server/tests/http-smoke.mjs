...
try {
  ...
  if (!unauthorized.headers.get('www-authenticate')?.includes('resource_metadata=')) {
    throw new Error('OAuth resource metadata was not advertised in the 401 response');
  }

  const issuer = `http://127.0.0.1:${port}`;
  const resource = `${issuer}/mcp`;
  const metadata = await fetch(`${issuer}/.well-known/oauth-authorization-server`).then(response => response.json());
  ...
  if (!registration.client_id) throw new Error(`OAuth DCR failed: ${JSON.stringify(registration)}`);
  ...
  if (consent.status !== 200 || !requestId) throw new Error('OAuth consent page was not rendered');
  ...
  if (approval.status !== 302 || callback.searchParams.get('state') !== 'smoke-state') {
  ...
  if (!tokenResponse.access_token || !tokenResponse.refresh_token) {
    throw new Error(`OAuth token exchange failed: ${JSON.stringify(tokenResponse)}`);
  }
  ...
  const oauthClient = new Client({ name: 'kraviona-oauth-smoke-test', version: '1.0.0' });
  await oauthClient.connect(oauthTransport);
  const oauthTools = await oauthClient.listTools();
  await oauthClient.close();
  if (oauthTools.tools.length !== tools.tools.length) throw new Error('OAuth access token could not access all tools');

  console.log(`HTTP MCP smoke test passed: Claude DCR + PKCE OAuth, static bearer auth, ${tools.tools.length} tools, and backend API access confirmed.`);
} finally {
  child.kill('SIGTERM');
}
