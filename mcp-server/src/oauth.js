...
class ClaudeClientsStore {
  ...
  registerClient(client) {
    if (
      ...
      !client.redirect_uris.every((uri) => CLAUDE_REDIRECT_URIS.has(uri))
    ) {
      throw new InvalidClientMetadataError(
        "Only the official Claude MCP callback URLs are allowed.",
      );
    }
...
<button type="submit">Authorize Claude</button></form><p class="note">Your backend admin password is not shared with Claude.</p></main></body></html>`);
  ...
  approve(requestId, suppliedKey, res) {
    ...
    if (!request || request.expiresAt < Date.now()) {
      ...
      return res
        .status(400)
        .send(
          "Authorization request expired. Return to Claude and connect again.",
        );
    }
  ...
  async challengeForAuthorizationCode(client, authorizationCode) {
    ...
    if (
      ...
      record.client.client_id !== client.client_id
    ) {
  ...
  async exchangeAuthorizationCode(
    ...
    return this.issueTokens(
      ...
      record.params.resource || this.resourceUrl,
    );
  }
  ...
  async exchangeRefreshToken(client, refreshToken, scopes, resource) {
    ...
    if (requestedScopes.some((scope) => !record.scopes.includes(scope))) {
      throw new InvalidScopeError(
        "Refresh request contains an unauthorized scope.",
      );
    }
