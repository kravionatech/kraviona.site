...
export default function PostCard({ post, featured = false, index }: { post: any; featured?: boolean; index?: number }) { const words=post.wordCount||plainText(post.content||'').split(/\s+/).filter(Boolean).length;
  return <article className={`story-card${featured ? ' story-card--featured' : ''}`}>
    ...
    <div className="story-card__content">
      <div className="story-card__meta"><a href={post.category?.slug ? `/category/${post.category.slug}` : '/blog'}>{post.category?.name || 'Journal'}</a><span>{formatDate(post.publishedAt)} · {readingTime(words)} min</span></div>
      <h2><a href={`/blog/${post.slug}`}>{post.title}</a></h2>
      <p>{post.quickAnswer}</p>
      <a className="read-link" href={`/blog/${post.slug}`}>Read story <span>↗</span></a>
    </div>
  </article>;
}
