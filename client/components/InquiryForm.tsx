  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch(`${API}/inquiries`, {
        ...
        body: JSON.stringify({ ...data, serviceName: selected?.title || "" }),
      });
  ...
  return (
    <form className="inquiry-form" onSubmit={submit}>
      ...
      <div className="inquiry-form__row">
        <label>
          Phone / WhatsApp
          <input name="phone" autoComplete="tel" placeholder="+91 …" />
        </label>
        <label>
          Company
          <input
      ...
      <label>
        ...
        <textarea
          ...
          maxLength={4000}
          rows={6}
          placeholder="Tell us the goal, current problem, and ideal timeline."
        />
      </label>
      ...
      <div className="inquiry-form__submit">
        ...
        <span>
          Direct to the Kraviona team · Usually replies within 1 business day
        </span>
      </div>
      {message && (
        <p
          className={`inquiry-message inquiry-message--${state}`}
          role="status"
        >
          {message}
        </p>
