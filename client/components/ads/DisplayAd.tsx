...
export default function DisplayAd({ size, className = '' }: DisplayAdProps) {
  ...
  return <section className={`ad-unit ad-unit--${size} ${className}`} aria-label="Advertisement"><span className="ad-unit__label">Advertisement</span><iframe title="Advertisement" loading="eager" scrolling="no" sandbox="allow-scripts allow-same-origin allow-popups allow-popups-to-escape-sandbox" src={`/ad/${size}`} width={unit.width} height={unit.height} /></section>;
}
