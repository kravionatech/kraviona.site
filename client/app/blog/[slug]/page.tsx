...
export async function generateMetadata({
  ...
  try {
    ...
    const title = truncate(p.seo?.metaTitle || p.title, 60);
    const description = truncate(
      p.seo?.metaDescription || p.quickAnswer || p.content,
      160,
    );
    const canonical = `/blog/${p.slug}`;
    const image = p.seo?.ogImage || p.featuredImage?.url || DEFAULT_OG_IMAGE;
    const authorName = p.author?.name || "Kraviona Editorial Team";
    return {
      title,
      description,
...
export default async function PostPage({
  ...
  if (p.category?._id)
    try {
      const d = await api(`/posts?category=${p.category._id}&limit=4`);
      related = d.items.filter((x: any) => x._id !== p._id).slice(0, 3);
    } catch {}
  const canonical = absoluteUrl(`/blog/${p.slug}`);
  const description = truncate(
    p.seo?.metaDescription || p.quickAnswer || p.content,
    160,
  );
  const image = absoluteUrl(
    p.seo?.ogImage || p.featuredImage?.url || DEFAULT_OG_IMAGE,
  );
  ...
  const authorName = p.author?.name || "Kraviona Editorial Team";
  const authorIsTeam = /team|editorial|kraviona/i.test(authorName);
  const author = {
    ...
    "@graph": [
      {
        ...
        image: {
          ...
          url: image,
          contentUrl: image,
          caption: p.featuredImage?.alt || p.title,
        },
        thumbnailUrl: image,
            ...
            ? [
                {
                  ...
                  item: `${SITE_URL}/category/${p.category.slug}`,
                },
              ]
            : []),
          {
            "@type": "ListItem",
            position: p.category?.slug ? 4 : 3,
            name: p.title,
            item: canonical,
          },
        ],
      ...
      <div className="wrap">
        <nav className="breadcrumbs" aria-label="Breadcrumb">
          ...
          {p.category?.slug && (
            <>
              <span>/</span>
              <a href={`/category/${p.category.slug}`}>{p.category.name}</a>
            </>
          )}
        </nav>
        <header className="article-header">
          ...
          <div className="byline">
            ...
            <span>
              {readingTime(effectiveWordCount)} min read · {effectiveWordCount}{" "}
              words
            </span>
          </div>
        </header>
        ...
        <div className="article-layout">
          <aside className="article-sidebar" aria-label="Article summary">
            ...
            <DirectLinkAd
              ...
              description="Explore verified technical solutions and partner offers."
              buttonText="Discover Offer ↗"
            />
          ...
          <div>
            ...
            <DisplayAd
              size="320x50"
              className="article-inline-ad ad-slot--mobile"
            />
          </div>
        </div>
        {p.faqs?.length > 0 && (
        ...
        {related.length > 0 && (
          ...
          >
            ...
            <div className="story-grid">
              {related.map((x: any, i: number) => (
                <PostCard post={x} index={i + 1} key={x._id} />
              ))}
            </div>
          </section>
        )}
