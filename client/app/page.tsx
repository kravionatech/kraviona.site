...
export default async function Home() {
  ...
  try { const [postData, categoryData, siteSettings] = await Promise.all([api('/posts?limit=10'), api('/categories'),api('/settings')]); posts=postData.items;categories=categoryData;settings=siteSettings; } catch {}
  ...
  return <>
    ...
    <section className="newsletter-band"><div className="wrap newsletter-band__inner"><div><div className="eyebrow">The weekly briefing</div><h2>{settings.briefingTitle||'Your inbox deserves better ideas.'}</h2></div><p>{settings.briefingDescription||'One original essay or practical framework every week. Thoughtful, concise, and worth keeping.'}</p><NewsletterForm compact/></div></section>
