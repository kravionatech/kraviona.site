...
export default async function RootLayout({ children }: { children: React.ReactNode }) {
  ...
  return <html lang="en-IN"><body>
    ...
    <header className="site-header">
      ...
      <div className="wrap nav"><a className="brand" href="/" aria-label={`${brand} home`}>{brand.toLowerCase()}<span>.</span></a><nav aria-label="Primary navigation"><a href="/blog">All stories</a>{categories.slice(0, 3).map(category => <a href={`/category/${category.slug}`} key={category._id}>{category.name}</a>)}<a href="/services">Services</a><a href="/guest-posting">Guest posting</a></nav><a className="header-cta" href="/services#contact">Work with us <span>→</span></a></div>
      <nav className="mobile-topics wrap" aria-label="Topics"><a href="/blog">All stories</a>{categories.map(category => <a href={`/category/${category.slug}`} key={category._id}>{category.name}</a>)}<a href="/services">Services</a><a href="/guest-posting">Guest posting</a></nav>
    </header>
