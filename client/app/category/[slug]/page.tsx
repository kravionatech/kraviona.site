...
async function getCategory(slug: string) {
  ...
  return cats.find((x: any) => x.slug === slug);
}
export async function generateMetadata({
  ...
  try {
    ...
    const suppliedDescription = plainText(
  ...
  const ld = [
    {
      ...
      mainEntity: {
        ...
        itemListElement: data.items.map((p: any, i: number) => ({
          "@type": "ListItem",
          position: i + 1,
          name: p.title,
    ...
    <>
            ...
            <div>
              <p>
                {category.description ||
                  `The latest Kraviona thinking about ${category.name.toLowerCase()}.`}
              </p>
              <span className="cover-count">
                {String(data.total).padStart(2, "0")} published{" "}
      ...
      <div className="wrap category-main">
        <div className="category-tabs">
          ...
          {categories.map((c) => (
            <a
              className={c._id === category._id ? "active" : ""}
        ...
        {featured ? (
          <>
            ...
            <a className="lead-story" href={`/blog/${featured.slug}`}>
              ...
              <div className="lead-copy">
                <span className="tag">{category.name} / Editors’ pick</span>
                <h2>{featured.title}</h2>
                <p>{featured.quickAnswer}</p>
                <div className="story-meta">
                  <span>
                    {Math.max(1, Math.ceil(featured.wordCount / 220))} min read
                  </span>
                  <b>Read the story ↗</b>
                </div>
              </div>
          ...
          <div className="empty-state">
            ...
            <h2>Our first {category.name} story is in progress.</h2>
      ...
      <div className="ad-slot--compact">
        <DisplayAd size="300x250" />
      </div>
      <script
