...
const Generated = z.object({
  ...
  keyTakeaways: z.array(z.string()).min(5),
  faqs: z.array(z.object({ question: z.string(), answer: z.string() })).min(7),
...
export async function generatePost({ topic, category, mode = "manual" }) {
  ...
  const response = await client.messages.create({
    ...
    messages: [
      {
        role: "user",
        content: `Create a useful, accurate, original article about: ${topic}`,
      },
    ],
  });
  ...
  let parsed;
  try {
    parsed = Generated.parse(
      JSON.parse(raw.replace(/^```json\s*|\s*```$/g, "")),
    );
  } catch {
    throw Object.assign(new Error("AI returned invalid post JSON"), {
      status: 502,
    });
  }
  ...
  if (wordCount < 800 || JSON.stringify(parsed).includes("undefined"))
    throw Object.assign(new Error("Generated post failed SEO validation"), {
      status: 422,
    });
  ...
  return Post.create({
    ...
    author: {
      ...
      sameAs: (process.env.AUTHOR_PROFILES || `${base}/about`)
        .split(",")
        .filter(Boolean),
    },
    seo: {
    ...
    generatedBy: mode === "auto" ? "ai-auto" : "ai-manual",
    status:
      mode === "auto" && process.env.AUTO_PUBLISH !== "false"
        ? "published"
        : "draft",
  });
}
