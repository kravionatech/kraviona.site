...
async function getClient() {
  if (!configuredUrl()) {
    throw Object.assign(new Error('Image uploads are unavailable: set CLOUDINARY_URL to a valid cloudinary:// URL'), { status: 503 });
  }
...
export async function uploadImage(dataUri, folder = 'kraviona') {
  if (!String(dataUri || '').startsWith('data:image/')) {
    throw Object.assign(new Error('A valid image file is required'), { status: 400 });
  }
