...
export async function requestSubscription(email) {
  ...
  if (process.env.RESEND_API_KEY) await new Resend(process.env.RESEND_API_KEY).emails.send({ from: process.env.NEWSLETTER_FROM || 'Kraviona <newsletter@kraviona.site>', to: email, subject: 'Confirm your subscription', html: `<p>Confirm your subscription:</p><p><a href="https://kraviona.site/newsletter/confirm?token=${confirmToken}">Confirm subscription</a></p>` });
  return subscriber;
}
