...
import { Post, Category, Comment, User, Subscriber, KeywordQueue, SiteSettings } from '../models/index.js';
import { auth, admin, commentLimiter } from '../middleware/index.js';
...
r.get('/dashboard',auth,admin,wrap(async(_,res)=>{const [posts,published,pendingComments,subscribers,recentAI,recentPosts]=await Promise.all([Post.countDocuments(),Post.countDocuments({status:'published'}),Comment.countDocuments({status:'pending'}),Subscriber.countDocuments({status:'subscribed'}),Post.find({generatedBy:{$ne:'manual'}}).sort({createdAt:-1}).limit(5),Post.find().populate('category','name').sort({updatedAt:-1}).limit(6)]);res.json({posts,published,drafts:posts-published,pendingComments,subscribers,recentAI,recentPosts});}));
